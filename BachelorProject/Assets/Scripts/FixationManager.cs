...
public class FixationManager : MonoBehaviour
{
    ...
    private void EvaluateFixation(GazePoint gazePoint)
    {
        ...
        //TODO: Also, right now, this check can be tricked by looking at the upper edge, then the sky, which does not trigger this, as the sky does not generate new gazePoints, thereby not clearing the current list and not creating a newFixation. The newFixation is only created when a new gazepoint is generated by looking at another object. In the meantime, the character can do a 180, causing the raycast (for normal detection) on Fixation Creation to not hit the underlying object. This needs to be fixed.
